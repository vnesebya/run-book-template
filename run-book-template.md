...
_ (например, '10 виртуальных машин с Ubuntu 14 AWS IaaS + 2 региона AWS + 2 VPC на регион + Route53 ') _
...
_ (например, мин: 4 виртуальных машины с 2 vCPU каждая. Максимально: около 40 виртуальных машин) _

### Требуемые ресурсы - хранилище

_ (например, мин. 10 ГБ хранилища BLOB-объектов Azure. Максимально: около 500 ГБ хранилища BLOB-объектов Azure) _

### Требуемые ресурсы - база данных

_ (например, мин. 500 ГБ стандартного уровня RDS. Максимально: около 2 ТБ стандартного RDS уровня) _
...
_ (например, мин: 60 строк журнала на узел в минуту (100 КБ). Максимально: около 6000 строк журнала на узел в минуту (1 МБ)) _
...
> Как происходит резервное копирование? Это влияет на обслуживание? Должна ли система [частично] быть предварительно отключена?

_ (например, резервное копирование происходит из реплики чтения - на действующую службу это не влияет) _

### Процедуры восстановления

> Как происходит восстановление? Это влияет на обслуживание? Должна ли система [частично] быть предварительно отключена?
